"""Test dual GPU bs=3 per GPU with proper synchronization."""
import os
...
def worker(gpu_id, results_queue):
    ...
    results_queue.put((gpu_id, peak, ms, toks))

if __name__ == "__main__":
    print("=== Dual GPU Benchmark: bs=3 per GPU (total bs=6) ===")
    ...
    processes = []
    for gpu_id in [0, 1]:
        p = ctx.Process(target=worker, args=(gpu_id, results_queue))
        p.start()
        processes.append(p)
    ...
    while not results_queue.empty():
        gpu_id, peak, ms, toks = results_queue.get()
        results[gpu_id] = (peak, ms, toks)
    ...
    if len(results) == 2:
        ...
        print(f"GPU 0: {results[0][0]:.0f}MB, {results[0][1]:.0f}ms/step")
        print(f"GPU 1: {results[1][0]:.0f}MB, {results[1][1]:.0f}ms/step")
        print(f"Total: {total_toks_per_sec/1000:.1f}K toks/sec")
        print(f"vs single GPU bs=3: {total_toks_per_sec / (24576*1000/852):.2f}x")
    else:
        print(f"Only {len(results)} GPU(s) completed")
