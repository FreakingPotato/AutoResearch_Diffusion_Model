"""Optimized dual GPU test with better data loading."""
import os
...
def worker(gpu_id, return_dict):
    ...
    BATCH_SIZE = 3
    # Use different num_workers per GPU to reduce contention
    loader = make_loader(BATCH_SIZE, "train", 8192, num_workers=2 if gpu_id == 0 else 2, prefetch_factor=2)
    ...
    # Warmup - consume data through pipeline
    for _ in range(5):
    ...
    # Benchmark 20 steps with stable measurement
    model.train()
...
if __name__ == "__main__":
    print("=== Optimized Dual GPU: bs=3 per GPU, num_workers=2 ===")
